...
from paths import checkpoint_file, ensure_runtime_dirs, get_root
...
# =============================================================================
def run_training(subj: str = "subj01", root: Path | None = None):
    """Full Stage 1 training run for one subject."""
    root = get_root(root)

    # ── Paths ─────────────────────────────────────────────────────────────────
    meta_dir  = root / "nsd_meta"
    ckpt_path = checkpoint_file(root, subj).parent
...
if __name__ == "__main__":
    ...
    run_training(subj=args.subj, root=get_root(args.root))
